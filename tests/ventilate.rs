...
fn test_ventilate() {
    ...
    let releve = Releve {
        ...
        total_des_operations_debit: 80.0,
        total_des_operations_credit: 2000.0,
        check_debit: 80.0,
        check_credit: 2000.0,
        operations: vec![
            Operation {
    ...
    assert_eq!(result.ventilation.get("Cirque"), Some(&50.0));
    assert_eq!(result.ventilation.get("Restaurant"), Some(&30.0));
...
fn test_ventilate_multiple_match_error() {
    ...
    let releve = Releve {
        ...
        total_des_operations_debit: 50.0,
        total_des_operations_credit: 0.0,
        check_debit: 50.0,
        check_credit: 0.0,
    ...
    match result.unwrap_err() {
        VentilateError::MultipleMatch { operation, matches } => {
            ...
            assert!(matches.contains(&"Cirque".to_string()));
            assert!(matches.contains(&"Soleil".to_string()));
        }
...
fn test_ventilate_error_on_second_operation() {
    ...
    let releve = Releve {
        ...
        total_des_operations_debit: 100.0,
        total_des_operations_credit: 0.0,
        check_debit: 100.0,
        check_credit: 0.0,
        operations: vec![
...
fn test_ventilate_not_assigned() {
    ...
    let releve = Releve {
        ...
        total_des_operations_debit: 200.0,
        total_des_operations_credit: 0.0,
        check_debit: 200.0,
        check_credit: 0.0,
        operations: vec![
    ...
    assert_eq!(result.ventilation.get("Cirque"), Some(&50.0));
    assert_eq!(result.not_assigned, 150.0); // 30 + 120 (RESTAURANT + SUPERMARCHE)
}
...
fn test_ventilate_sum_mismatch() {
    ...
    let releve = Releve {
        ...
        total_des_operations_debit: 100.0, // Incorrect: actual debits sum to 50.0
        total_des_operations_credit: 0.0,
        check_debit: 100.0,
        check_credit: 0.0,
        operations: vec![Operation {
